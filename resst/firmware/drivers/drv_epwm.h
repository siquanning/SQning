 ...
 * Also re-enables OST interrupt (TZEINT.OST=1) so future trips are detected.
...
/*
 * Disable OST interrupt without clearing the trip latch.
 * Used in TZ ISR to prevent re-entry while OST remains latched.
 * TZEINT.OST is re-enabled by DrvEpwm_ClearOstTrip().
 */
