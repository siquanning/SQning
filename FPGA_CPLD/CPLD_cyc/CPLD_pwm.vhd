...
architecture structural of CPLD_pwm is
    ...
    -- A-phase H1 activity filter used only as the H2 enable guard. Short
    -- commutation gaps are ignored; 300 cycles at 150 MHz equals 2 us.
    constant H1_OFF_FILTER_CYCLES : integer := 300;
    signal h1_a_zero_count : integer range 0 to H1_OFF_FILTER_CYCLES := 0;
    signal h1_a_active     : std_logic := '0';
    signal h1_a_guard      : std_logic_vector(3 downto 0);
...
begin
    ...
    -- =========================================================
    process(clk_150m, rst_n)
    begin
        if rst_n = '0' then
            h1_a_zero_count <= 0;
            h1_a_active <= '0';
        elsif rising_edge(clk_150m) then
            if sync_h1(3 downto 0) /= "0000" then
                h1_a_zero_count <= 0;
                h1_a_active <= '1';
            elsif (h1_a_active = '1') and
                  (h1_a_zero_count < H1_OFF_FILTER_CYCLES) then
                h1_a_zero_count <= h1_a_zero_count + 1;
            else
                h1_a_zero_count <= H1_OFF_FILTER_CYCLES;
                h1_a_active <= '0';
            end if;
        end if;
    end process;

    h1_a_guard <= "0001" when h1_a_active = '1' else "0000";
