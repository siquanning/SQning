...
(async () => {
  ...
  const remainingText = await page.locator("#remainingCount").innerText();
  if (!remainingText.startsWith("剩余 ")) {
    throw new Error(`remaining count did not render correctly: "${remainingText}"`);
  }
  if (errors.length > 0) {
