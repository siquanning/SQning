...
vm.runInContext(`
  ...
  game.lastDraw = { playerId: 0, tile: t("character", 9) };
  ...
  sortTiles(game.players[1].hand);
  game.lastDraw = { playerId: 1, tile: t("character", 9) };
  game.players[1].hand.push(cloneTile(game.lastDraw.tile));
  sortTiles(game.players[1].hand);
  const shouldPiao = shouldPiaoCai(1, game);
  if (typeof shouldPiao !== "boolean") throw new Error("shouldPiaoCai should return boolean");
