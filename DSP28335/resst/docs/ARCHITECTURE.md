# ARCHITECTURE — F28335_RTControl_Platform

## 1. Project Purpose

F28335_RTControl_Platform is a real-time control platform firmware for the TMS320F28335 DSP. It currently operates as a UART→SPI bridge: receives variable-length UART frames over SCI-A, parses them into complete frames, and forwards each frame byte-by-byte over SPI-A to a CPLD slave. The project is a clean-layered refactor of the legacy `DROOP_SPI_UART` codebase, formerly known as `DROOP_SPI_UART_REFACTOR`.

**Target**: TMS320F28335, 150 MHz SYSCLKOUT, EABI (ELF) toolchain.  
**Compiler**: TI C2000 CGT 25.11.0.LTS (`cl2000 -v28`).  
**Builds**: Four identities — Prototype_RAM_Debug, Prototype_Flash_Demo, Industrial_RAM_Debug, Industrial_Flash_Release (see [BUILD_AND_FLASH.md](BUILD_AND_FLASH.md)).
