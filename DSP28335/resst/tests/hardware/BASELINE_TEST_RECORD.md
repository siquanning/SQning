# BASELINE_TEST_RECORD — DROOP_SPI_UART_REFACTOR（历史基线）

> **⚠ 历史基线记录**: 本文档及 `baseline_artifacts/` 目录下的所有测试记录、产物名（如 `DROOP_SPI_UART_REFACTOR.out/.map`）均来自改名前的历史基线。当前平台已更名为 **F28335_RTControl_Platform**，当前构建产物使用新名称。本文档中的旧名称保留为历史来源标识，不代表当前项目名。
