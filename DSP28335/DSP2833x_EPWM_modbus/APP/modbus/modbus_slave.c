/*
 * modbus_slave.c — Modbus RTU slave protocol implementation
 */
...
{
    if (rx_cnt < 4) return;
    if (rx_buf[0] != MODBUS_ADDR) return;
...
void MB_Poll(void)
{
    if (rx_cnt > 0 && (ms_counter - last_rx_time) >= 5)   // 5ms 帧间隔 ≈3.5 字符 @9600
    {
        SciaRegs.SCIFFRX.bit.RXFFIENA = 0;   // 关 RX 中断，防止 ISR 与主循环竞态
        ProcessFrame();
        rx_cnt = 0;
