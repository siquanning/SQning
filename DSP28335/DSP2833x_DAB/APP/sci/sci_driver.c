/*
 * sci_driver.c — SCI-A 驱动，FIFO 中断接收
 *
 * 接收路径：ISR → MB_FeedByte() → Modbus 状态机
 * 发送：阻塞查询方式（对短 Modbus 响应足够）
 */
...
void Init_Scia(void)
{
    ...
    // 退出复位，启动 SCI（SWRESET=1）
    SciaRegs.SCICTL1.all = 0x0023;

    // IER/PIEIER — 全局 EINT 由 main() 最后统一开启
    IER |= M_INT9;
